...
import {
  ...
  getFavourCity,
  favourCity,
} from "../../redux/actionCreator/CityDataCreator";
...
function Data(props) {
  ...
  useEffect(() => {
    if (
      allAQData.length === 0 ||
      allAQData[0].hour.toString() !== moment().subtract(2, "hours").format("H")
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      prevFavourNum !== favourCityList.length
    ) {
      getFavourCity(token);
  ...
  useEffect(() => {
    if (Array.isArray(allAQData)) {
      setDataSource(
        allAQData.map((item, index) => {
          const city = cityList.find((city) => city.id === item.city_id);
          var favour = false;
          if (favourCityList !== false) {
            favour = favourCityList.find(
              (favour) => favour.city_id === item.city_id
            );
          }
          return {
            key: index + 1,
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <Button
          icon={
            <StarFilled
              style={{ color: text[0] === "t" ? "gold" : "grey" }}
              onClick={() => {
                favourCity({
